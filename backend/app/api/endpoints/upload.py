...
from app.core.security import require_trainer_or_admin, get_current_user
...
async def upload_video(
    ...
    if size > settings.MAX_VIDEO_SIZE:
        raise HTTPException(status_code=400, detail=f"File too large. Max size: {settings.MAX_VIDEO_SIZE // (1024*1024)}MB")
...
async def upload_document(
    ...
    if size > settings.MAX_DOC_SIZE:
        raise HTTPException(status_code=400, detail=f"File too large. Max size: {settings.MAX_DOC_SIZE // (1024*1024)}MB")
