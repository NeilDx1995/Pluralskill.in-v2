...
async def get_workshops(
    ...
    skip = (page - 1) * limit
    workshops = await db.workshops.find(query, {"_id": 0}).skip(skip).limit(limit).to_list(limit)
...
@router.post("/")
async def create_workshop(workshop_in: WorkshopCreate, user: dict = Depends(require_admin)):
