...
def _configure_gemini():
    ...
    if not settings.GEMINI_API_KEY:
        raise RuntimeError("Gemini API key not configured. Get one free at https://aistudio.google.com/apikey")
    if not _gemini_configured:
        genai.configure(api_key=settings.GEMINI_API_KEY)
        _gemini_configured = True

async def generate_learning_path(skill_name: str, current_level: str, industry: str, goal: str):
    _configure_gemini()
            ...
            "steps": [
                {
                    ...
                    "resources": [
                        { "title": "Official Documentation", "url": f"https://google.com/search?q={skill_name}+documentation", "type": "article", "description": "Official docs" }
                    ]
                },
                {
                    "week": 2,
                    ...
                    "resources": [
                        { "title": "YouTube Tutorials", "url": f"https://youtube.com/results?search_query={skill_name}+tutorial", "type": "video", "description": "Video tutorials" }
                    ]
                }
            ],
