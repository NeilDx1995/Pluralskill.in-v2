...
class TestStudentEnrollmentAndProgress:
    ...
    def test_get_courses_list(self):
        ...
        assert (
            len(finance_course.get("modules", [])) > 0
        ), "Finance course has no modules"
        assert (
            len(finance_course.get("tests", [])) > 0
        ), "Finance course has no quiz questions"
        print(
            f"✓ Found Finance course with {len(finance_course['modules'])} modules and {len(finance_course['tests'])} quiz questions"
        )
    ...
    def test_student_enrolled_courses(self):
        ...
        response = requests.get(
            f"{BASE_URL}/api/my-courses", headers=TestSetup.get_auth_header(token)
        )
        assert response.status_code == 200
        courses = response.json()
...
class TestModuleCompletion:
    ...
    def test_mark_module_complete(self):
        ...
        print(
            f"✓ Module marked complete, overall progress: {result['overall_progress']}%"
        )
...
class TestQuizSystem:
    ...
    def test_submit_quiz_passing_score(self):
        ...
        response = requests.post(
            ...
            json={"course_id": course_id, "answers": answers},
            headers=TestSetup.get_auth_header(token),
        )
        ...
        if response.status_code == 400 and "attempts" in response.text.lower():
            print(
                "✓ Quiz submission blocked - max attempts reached (expected behavior)"
            )
            return
        ...
        print(
            f"✓ Quiz submitted: score={result['score']}%, passed={result['passed']}, attempts_remaining={result['attempts_remaining']}"
        )
    ...
    def test_quiz_attempt_limit(self):
        ...
        for test in tests:
            # Pick wrong answer
            wrong_answers[test["id"]] = (test["correct_answer"] + 1) % len(
                test["options"]
            )
        ...
        assert (
            result1["attempts_remaining"] == 1
        ), f"Expected 1 attempt remaining, got {result1['attempts_remaining']}"
        print(
            f"✓ Attempt 1: score={result1['score']}%, attempts_remaining={result1['attempts_remaining']}"
        )
        ...
        assert (
            result2["attempts_remaining"] == 0
        ), f"Expected 0 attempts remaining, got {result2['attempts_remaining']}"
        print(
            f"✓ Attempt 2: score={result2['score']}%, attempts_remaining={result2['attempts_remaining']}"
        )
        ...
        ), f"Expected 400 for 3rd attempt, got {response3.status_code}"
        assert (
            "attempts" in response3.text.lower() or "maximum" in response3.text.lower()
        )
...
class TestAssignments:
    ...
    def test_trainer_create_assignment(self):
        ...
        courses = requests.get(
            f"{BASE_URL}/api/trainer/courses", headers=TestSetup.get_auth_header(token)
        ).json()
        ...
        ), f"Failed to create assignment: {response.text}"
...
class TestCertificates:
    ...
    def test_get_my_certificates(self):
        ...
        response = requests.get(
            f"{BASE_URL}/api/certificates", headers=TestSetup.get_auth_header(token)
        )
        assert response.status_code == 200
    ...
    def test_full_learning_flow(self):
        ...
        course = next(
            (
                c
                for c in courses
                if len(c.get("tests", [])) > 0 and len(c.get("modules", [])) > 0
            ),
            None,
        )
        ...
        assert (
            enroll_response.status_code == 200
        ...
        for module in course["modules"]:
            ...
            ), f"Module completion failed: {complete_response.text}"
        ...
        quiz_response = requests.post(
            f"{BASE_URL}/api/progress/quiz/submit",
            json={"course_id": course["id"], "answers": correct_answers},
            headers=headers,
        )
        assert (
            quiz_response.status_code == 200
        ), f"Quiz submission failed: {quiz_response.text}"
        quiz_result = quiz_response.json()
        assert (
            quiz_result["passed"] == True
        ), f"Quiz not passed: score={quiz_result['score']}%"
        ...
        if quiz_result.get("certificate_issued"):
            ...
            verify_response = requests.get(
                f"{BASE_URL}/api/certificates/verify/{cert['certificate_number']}"
            )
